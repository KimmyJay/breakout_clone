    ...
    def create_wall(self):
        ...
        for row in range(rows):
            ...
            for col in range(cols):
                ...
                rect = pygame.Rect(block_x, block_y, self.width,
                                   self.height)  # create rect value based on x,y coordinates and width and height value
...
class game_ball():
    ...
    def __init__(self, x, y):
        #calls the reset function that reset the ball's variables each time an instance of game_ball is created
        self.reset(x, y)
    ...
    def move(self):
        ...
        if self.rect.colliderect(player_paddle):
            ...
            # collision occurs from left or right of paddle
            else:
                self.speed_x *= -1
    ...
    #resetting all variables back to the original ones
    def reset(self, x, y):
...
while run:
    ...
    #paddle and ball only allowed to move when ball is live
    if live_ball:
        player_paddle.move()
        game_over = ball.move()
        if game_over != 0:
    ...
    #print player instructions when game is not live
    if not live_ball:
        #when game is first initialized
        if game_over == 0:
            draw_text('CLICK ANYWHERE TO START', font, text_col, 100, screen_height // 2 + 100)
        #when all blocks have been destroyed
        elif game_over == 1:
            draw_text('YOU WON!', font, text_col, 240, screen_height // 2 + 50)
            draw_text('CLICK ANYWHERE TO START', font, text_col, 100, screen_height // 2 + 100)
        #when ball has left the bottom of screen
        elif game_over == -1:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            run = False
        #add an event for starting game when mouse is clicked and ball is not live
